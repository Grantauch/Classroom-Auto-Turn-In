...
(async()=>{
  ...
  try{
    ...
    const gradingPage=await context.newPage();
    await gradingPage.goto('https://classroom.google.com/c/COURSE_TEST/sp/TEACHER/all/default',{waitUntil:'domcontentloaded',timeout:15000});
    let resolveGradingPick;
    const gradingPicked=new Promise(resolve=>resolveGradingPick=resolve);
    const gradingBridge=await createClassroomPickerBridge(context,payload=>resolveGradingPick(payload),{purpose:'grading'});
    if(await gradingBridge.inject()!==1)throw new Error('Dedicated grading Classroom picker did not appear.');
    await gradingPage.locator('#cati-classroom-picker button').filter({hasText:'Add grading Classroom'}).click({timeout:5000});
    const gradingPayload=await Promise.race([gradingPicked,new Promise((_,reject)=>setTimeout(()=>reject(new Error('Dedicated grading Classroom picker did not return the teacher choice.')),5000))]);
    if(!String(gradingPayload.url||'').includes('/c/COURSE_TEST'))throw new Error('Dedicated grading Classroom picker returned the wrong course.');
    gradingBridge.dispose();
