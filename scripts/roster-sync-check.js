...
const bridge=validateBridge({url:'https://script.google.com/a/macros/example.org/s/DEPLOYMENT/exec',contract:'2026-09-22-roster-sync-v1',writeContract:'2026-09-22-roster-write-v1'});
assert.equal(bridge.contract,'2026-09-22-roster-sync-v1');assert.equal(bridge.writeContract,'2026-09-22-roster-write-v1');assert.equal(new URL(teacherUrl(bridge.url)).searchParams.get('mode'),'teacher');
...
const writeRequest=validateWriteRequest({confirmation:'APPLY SAFE ROSTER CHANGES',requestId:'gcr-test-request-123',baseRevision:REVISION_A,add:[{studentEmail:'ben@school.org',studentName:'Ben Student',classPeriod:'Period 3'}],updateName:[{studentEmail:'ada@school.org',studentName:'Ada Student',beforeName:'Ada Old Name',classPeriod:'Period 3'}]});
...
assert.throws(()=>validateWriteRequest({...writeRequest,add:[{studentEmail:'long@school.org',studentName:'X'.repeat(121),classPeriod:'Period 3'}],updateName:[]}),/student name.*too long/i,'Write requests must reject overlong identity data instead of truncating it.');
...
function makeHarness({failFirstApply=false}={}){
  ...
  secure.set('roster-sync.secure.json',{schemaVersion:1,lastDiscoveryAt:'2026-09-22T18:00:00Z',snapshot:complete,lastDiff:{counts:{}},issues:[]});plain.set('roster-mappings.json',mappings);
  ...
  const service=createRosterService({localData,secureData,ensureAutomationIdle:()=>{},compactError:e=>String(e),runExclusiveBrowser:async(_label,fn)=>fn(),runNodeScript:async(file,args)=>{
    ...
    if(file==='apply-operations-roster.js'){
      applyAttempts++;const packed=decodeBridgeArg(args[0]),req=validateWriteRequest(packed.request);requestIds.push(req.requestId);
