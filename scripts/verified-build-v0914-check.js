...
assert.strictEqual(lib.assignmentEligibility({ dueText: 'Due Sep 20' }, null, weekdays, fri).eligible, true, 'Sunday due date is not turned in at the Friday check');
assert.strictEqual(lib.assignmentEligibility({ dueText: 'Due Sep 19' }, null, weekdays, fri).eligible, true, 'Saturday due date is not turned in at the Friday check');
assert.strictEqual(lib.assignmentEligibility({ dueText: 'Due Sep 21' }, null, weekdays, fri).eligible, false, 'Monday due date must wait for the Monday check');
assert.strictEqual(lib.assignmentEligibility({ dueText: 'Due Sep 20' }, null, weekdays, wed).eligible, false, 'a Sunday due date is not due on Wednesday');
assert.strictEqual(lib.assignmentEligibility({ dueText: 'Due Sep 15' }, null, weekdays, wed).eligible, false, 'past due must wait for overdue catch-up');
assert.strictEqual(lib.assignmentEligibility({ dueText: 'Due Sep 15' }, null, { ...weekdays, submitOverdue: true }, wed).eligible, true);
const noDue = lib.assignmentEligibility({ dueText: 'No due date' }, null, weekdays, wed);
