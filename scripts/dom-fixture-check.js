...
rows=runFixture('assignment-detail-due-metadata',`<main role="main"><div><h1>Week 3 - Lesson Plans</h1><div>Principal Office • Sep 1</div><div>100 points</div><div aria-label="Due 9/30/2026, 11:59 PM">Due 9/30/2026, 11:59 PM</div><div>Attach this week's lesson plan and turn it in.</div><h3>Class comments</h3><div>Due 10/10/2026, 11:59 PM</div></div></main>`,`const fn=${dueEvidenceFn};const r=fn();document.documentElement.setAttribute('data-result',encodeURIComponent(JSON.stringify(r)));`);
if(rows.length!==1||rows[0]!=='Due 9/30/2026, 11:59 PM')throw new Error(`Assignment detail due-date collector must accept a semantically labelled deadline field: ${JSON.stringify(rows)}`);
rows=runFixture('assignment-detail-body-div-due-decoy',`<main role="main"><div><h1>Week 3 - Lesson Plans</h1><div>Principal Office • Sep 1</div><div>100 points</div><div>Due 9/30/2026, 11:59 PM</div><h3>Class comments</h3><div>Due 10/10/2026, 11:59 PM</div></div></main>`,`const fn=${dueEvidenceFn};const r=fn();document.documentElement.setAttribute('data-result',encodeURIComponent(JSON.stringify(r)));`);
if(rows.length!==0)throw new Error(`Plain assignment body div text must not become verified due-date evidence: ${JSON.stringify(rows)}`);
rows=runFixture('assignment-detail-attribute-due-decoy',`<main role="main"><div><h1>Week 3 - Lesson Plans</h1><div>Principal Office • Sep 1</div><div>100 points</div><div title="Due 9/30/2026, 11:59 PM">Instructions mention Due 9/30/2026, 11:59 PM for another task.</div></div></main>`,`const fn=${dueEvidenceFn};const r=fn();document.documentElement.setAttribute('data-result',encodeURIComponent(JSON.stringify(r)));`);
if(rows.length!==0)throw new Error(`A due-looking attribute on instructional text must not become verified due-date evidence: ${JSON.stringify(rows)}`);
rows=runFixture('assignment-detail-time-only-plus-body-date',`<main role="main"><div><h1>Week 3 - Lesson Plans</h1><div>Principal Office • Sep 1</div><div>100 points</div><div aria-label="Due 8:00 AM">Due 8:00 AM</div><div>Due Sep 22, 2026</div></div></main>`,`const fn=${dueEvidenceFn};const r=fn();document.documentElement.setAttribute('data-result',encodeURIComponent(JSON.stringify(r)));`);
if(rows.length!==0)throw new Error(`Time-only deadline metadata plus a body date must remain unresolved: ${JSON.stringify(rows)}`);
rows=runFixture('assignment-detail-no-due-date',`<main role="main"><div><h1>Week 3 - Lesson Plans</h1><div aria-label="No due date">No due date</div></div></main>`,`const fn=${dueEvidenceFn};const r=fn();document.documentElement.setAttribute('data-result',encodeURIComponent(JSON.stringify(r)));`);
if(rows.length!==1||rows[0]!=='No due date')throw new Error(`Semantically labelled no-due-date state must remain readable: ${JSON.stringify(rows)}`);
